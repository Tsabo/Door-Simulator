...
public class Device : IDisposable
{
    ...
    public bool IsConnected => _connectionListener?.ConnectionCount > 0 && (
        _lastValidReceivedCommand + _deviceConfiguration.ConnectionTimeout >= DateTime.UtcNow);
    ...
    private async Task ClientListenLoop(IOsdpConnection incomingConnection)
    {
        try
        {
                ...
                incomingConnection, _deviceConfiguration.SecurityKey, _deviceConfiguration.Identification.ToBytes(), _loggerFactory)
            {
                ...
                ConnectionTimeout = _deviceConfiguration.ConnectionTimeout,
            };
...
public class DeviceConfiguration : ICloneable
{
    ...
    /// <summary>
    /// How long the ACU may go without a valid poll before <see cref="Device.IsConnected"/>
    /// reports disconnected. Upstream hardcodes this to 8 seconds, which is too tight for
    /// slower-polling panels (e.g. Mercury MP1502, observed polling at ~3s intervals with
    /// real-world jitter) and caused IsConnected to flicker false even on a healthy link.
    /// </summary>

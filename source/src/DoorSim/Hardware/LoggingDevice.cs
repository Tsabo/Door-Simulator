...
internal sealed class LoggingDevice : Device
{
    ...
    private readonly OsdpComsetBehavior _comsetBehavior;
    private readonly int _doorId;

    private readonly Func<bool> _getDoorOpen;
    private readonly Func<bool> _getRexActive;
    private readonly DeviceIdentification _identification;
    private readonly string _label;
    private readonly ILogger _logger;
    private readonly bool _nakManufacturerCommand;

    // Replies for the osdp_CAP and osdp_ID handshake, built once per door from its advanced
    // OSDP settings. These are operator-declared, NOT a description of what the simulator
    // actually supports — a door may deliberately over-state its capabilities to test how a
    // panel reacts. Per-instance rather than static: making them static again would silently
    // re-share one door's declared personality across every door.
    private readonly DeviceCapabilities _pdCapabilities;
    ...
    private volatile byte _osdpAddress;

    public LoggingDevice(DeviceConfiguration config,
        ...
        : base(config, loggerFactory)
    {
        ...
        _nakManufacturerCommand = door.OsdpNakManufacturerCommand;
    ...
    /// <remarks>
    /// The base class inspects this reply: a pdCOMMSET reply whose address differs from the
    /// current one causes it to update its own device configuration, which the connection loop
    /// re-reads on every incoming connection. That update happens through a private method, so
    /// it cannot be undone from here — which is why an accepted address change unavoidably
    /// takes effect on the next reconnect, and why the enum member is named for that rather
    /// than for merely acknowledging. The listener is never restarted, so the requested baud
    /// rate is echoed back but never applied.
    /// </remarks>
    protected override PayloadData HandleCommunicationSet(CommunicationConfiguration config)
    {
        if (_comsetBehavior == OsdpComsetBehavior.Nak)
        {
            _logger.LogInformation(
                "Door {Id} ({Label}): CP sent osdp_COMSET (address={Addr}, baud={Baud}) — replying Nak (COMSET declared unsupported)",
                _doorId, _label, config.Address, config.BaudRate);

            return new Nak(ErrorCode.UnknownCommandCode);
        }

        var accept = _comsetBehavior == OsdpComsetBehavior.AcceptAddress;
        var address = accept
            ? config.Address
            : _osdpAddress;
        ...
        if (accept && address != _osdpAddress)
        {
            _logger.LogWarning(
                "Door {Id} ({Label}): accepted osdp_COMSET address {OldAddr} -> {NewAddr} (baud {Baud} echoed but NOT applied). "
                + "The new address takes effect on the next connection; the stored door configuration still says {StoredAddr} — update it to persist this.",
                _doorId, _label, _osdpAddress, address, config.BaudRate, _osdpAddress);

            _osdpAddress = address;
        }
        else
        {
            _logger.LogDebug(
                "Door {Id} ({Label}): CP sent osdp_COMSET (address={Addr}, baud={Baud}) — {Behavior}; replying address={ReplyAddr}, baud={ReplyBaud}",
                _doorId, _label, config.Address, config.BaudRate, _comsetBehavior, address, baudRate);
        }

        // Must return the ReplyData type (pdCOMMSET reply), not the CommandData type.
        return new OSDP.Net.Model.ReplyData.CommunicationConfiguration(address, baudRate);
    }

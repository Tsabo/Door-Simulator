...
public class DoorConfigService(DoorSimDbContext db)
{
    ...
    public async Task<string[]> GetAvailableSerialPortsAsync(int? excludeDoorId = null)
    {
        var usedPorts = (await db.Doors
                .Where(d => excludeDoorId == null || d.Id != excludeDoorId)
                .Where(d => d.Protocol == nameof(ProtocolType.Osdp))
                .Select(d => d.OsdpSerialPort)
                .ToListAsync()
                .ConfigureAwait(false))
    ...
    public async Task<(DoorConfiguration? Result, string? Error)> UpdateAsync(int id, DoorConfiguration dto)
    {
        ...
        var error = await ValidateAsync(dto, excludeId: id).ConfigureAwait(false);
        if (error is not null) return (null, error);
    ...
    {
        if (dto.Protocol == ProtocolType.Osdp &&
            dto.OsdpBaudRate.HasValue &&
        ...
        var usedPins = others
            .SelectMany(d => new[] { d.D0Pin, d.D1Pin, d.DpsPin, d.RexPin })
            .OfType<int>()
            .ToHashSet();
        ...
        if (dto.ModbusTcpHost is null && dto.ModbusSerialPort is not null && dto.ModbusUnitId.HasValue)
        {
            var boardPeers = others
                .Where(d =>
                    d.ModbusTcpHost == null &&
                    d.ModbusSerialPort == dto.ModbusSerialPort &&
                    d.ModbusUnitId == dto.ModbusUnitId)
                .ToList();

            var usedChannels = boardPeers
                .SelectMany(d => new[] { d.DpsModbusChannel, d.RexModbusChannel })
                .OfType<int>()
                .ToHashSet();
        ...
        if (dto.ModbusTcpHost is not null && dto.ModbusUnitId.HasValue)
        {
            ...
            var boardPeers = others
                .Where(d =>
                    string.Equals(d.ModbusTcpHost, dto.ModbusTcpHost, StringComparison.OrdinalIgnoreCase) &&
                    (d.ModbusTcpPort ?? 502) == tcpPort &&
                    d.ModbusUnitId == dto.ModbusUnitId)
                .ToList();
    ...
    private static string[] DiscoverSerialPorts()
    {
        ...
        try
        {
            return Directory.EnumerateFiles("/dev", "tty*")
                .Where(path =>
                {
